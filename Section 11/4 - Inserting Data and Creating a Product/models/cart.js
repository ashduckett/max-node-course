...
module.exports = class Cart {
    ...
    static deleteProduct(id, productPrice) {
        ...
        fs.readFile(p, (err, fileContent) => {
            ...
            // Find the product with the ID passed in in the cart.
            const product = updatedCart.products.find(prod => prod.id === id);

            // If the ID doesn't match an item, return.
            if (!product) {
                return;
            }

            // Get hold of the product's quantity in the cart
            const productQty = product.qty;
            

            // Update the in-memory cart to remove the product with the id passed in
            // This may be more than one so will be all of the specific type
            updatedCart.products = updatedCart.products.filter(prod => prod.id !== id);

            // Update the price on the cart to remove all items' price from the total
